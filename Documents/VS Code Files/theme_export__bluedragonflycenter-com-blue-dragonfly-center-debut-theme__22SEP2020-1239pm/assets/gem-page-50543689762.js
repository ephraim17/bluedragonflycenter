  ...
  if (mode == 'dev') {
      ...
      if (flag) {
          ...
          jQuery(function() {
  var mode = jQuery('.gryffeditor').hasClass('editing') ? 'dev' : 'production';
  var $module = jQuery('#m-1594972822690').children('.module');
  if (mode == 'dev') {
      jQuery('#m-1594972822690').attr('data-name', '').css('background-image', 'none').removeAttr('data-image');
      
      var flag = true;
      var $bkLiquid = parent.jQuery('body').find('#gfFrame').contents().find('#module-1594972822690');
      if ($bkLiquid && $bkLiquid.length > 0) {
          var $settings = $bkLiquid.find('.settings');
          try {
              var name = '';
              ...
              if (imageUrl != '') {
                  flag = false;
                  jQuery('#m-1594972822690').css('background-image', 'url(' + imageUrl + ')').css('min-height', '100px').attr('data-image', 'true');
              }
              if (name != '' && name != 'Custom Code') {
                  flag = false;
                  jQuery('#m-1594972822690').attr('data-name', name);
              }
          } catch(error) {
              console.log(error);
          }
      ...
      if (flag) {
          jQuery('#m-1594972822690').attr('data-name', 'Right click on the module, then choose Edit Html / Liquid option to start writing your custom code.');
      }
  }
});
